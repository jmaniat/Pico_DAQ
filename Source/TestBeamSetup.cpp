...
void TestBeamSetup::WriteFile(){
    ...
    tot_val.open("./ToT_values.txt");
    
    std::map<std::string, std::vector<double>> output_map;
    std::map<std::string, std::vector<double>> output_map_tmp;
    output_map = Detectors.at(0)->GetToT();
    output_map_tmp = Detectors.at(1)->AddRef();
    output_map.insert(output_map_tmp.begin(), output_map_tmp.end());
    
    
    for(int i = 0; i < output_map["first_0"].size(); i++){
        if(i == 0) std::cout << output_map["Expo_Charge"].at(i) << std::endl;
        tot_val << output_map["V_max"].at(i) << "\t" << output_map["Peak_Charge"].at(i) << "\t" << output_map["Expo_Charge"].at(i) << "\t" << output_map["Ref"].at(i) << "\t" << output_map["Time_naive"].at(i) << "\t" << output_map["first_0"].at(i) << "\t" << output_map["second_0"].at(i) << "\t" << output_map["first_1"].at(i) << "\t" << output_map["second_1"].at(i) << "\t" << output_map["first_2"].at(i) << "\t" << output_map["second_2"].at(i) << "\t" << output_map["first_3"].at(i) << "\t" << output_map["second_3"].at(i) << "\t" << output_map["first_4"].at(i) << "\t" << output_map["second_4"].at(i) << "\t" << output_map["first_5"].at(i) << "\t" << output_map["second_5"].at(i) << std::endl;
    }
