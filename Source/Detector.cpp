...
void Detector::FindNaiveTiming()
{
    ...
    for(int i = global_maximum.position ; i > start_point.position && i>1; --i)
    {
        if( waveform_y.at(i) - cf > 0 && waveform_y.at(i-1) - cf < 0 )
        {
            ...
            //            std::cout << "Naive timing = " << naive_time << std::endl;
            break;
        }
    }
}
...
void Detector::ExpoFit()
{
    ...
    TLine *l = new TLine(ToT_fit.at(5), 0, ToT_fit.at(5),1.2);
    TLine *l_hor = new TLine(16, threshold.at(5), 24, threshold.at(5));
...
void Detector::TimeSigmoid()
{
    ...
    for(int i = 0; i < threshold.size(); i++){
        ToT_fit.push_back(Sigmoid.parameters[1]-(TMath::Log((Sigmoid.parameters[0]/(threshold.at(i)-Sigmoid.parameters[3]))-1)/Sigmoid.parameters[2]));
    }
    
}

void Detector::ToT(int detector_type){
    
    if(detector_type == 1){
        ToT_values["Ref"].push_back(ref_naive_time);
        return;
    }
    float ampl_val;
    ToT_values["V_max"].push_back(global_maximum.y);
    ToT_values["Peak_Charge"].push_back(charge_e_peak);
    ToT_values["Expo_Charge"].push_back(charge_expo_full);
    ToT_values["Time_naive"].push_back(naive_time);
    for(int i = 0; i < threshold.size(); i++){
        ToT_values[Form("first_%i",i)].push_back(ToT_fit.at(i) > global_maximum.y ? ToT_fit.at(i) : -1);
        for(int j = global_maximum.position; j < waveform_y.size(); j++){
            if(waveform_y.at(j) > threshold.at(i)) continue;
            if(ToT_values[Form("first_%i",i)].back() != -1){
                if(waveform_x.at(j) - ToT_values[Form("first_%i",i)].back() > 0.05) ToT_values[Form("second_%i",i)].push_back(waveform_x.at(j));
                else ToT_values[Form("second_%i",i)].push_back(-1);
            }
            else{
                ToT_values[Form("second_%i",i)].push_back(-1);
            }
            if(ToT_values[Form("second_%i",i)].back() == -1) ToT_values[Form("first_%i",i)].back() = -1;
            
            //            ToT_values[Form("second_%i",i)].push_back((ToT_values[Form("first_%i",i)].back() != -1 && ToT_values[Form("first_%i",i)].back() > global_maximum.y )? waveform_x.at(j) : -1);
            break;
        }
    }
}

std::map<std::string, std::vector<double>> Detector::GetToT() {
...
std::map<std::string, std::vector<double>> Detector::AddRef(){
    return ToT_values;
}
